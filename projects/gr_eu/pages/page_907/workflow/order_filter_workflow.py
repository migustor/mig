...
@jenkins_aware()
def filter_and_get_orders_workflow(driver, timeouts=None):
    """
    1) Set order status filter to 'delivered'
    2) Build report
    3) Retrieve first three order IDs
    """
    logger = logging.getLogger(' - TEST - ')
    logger.info("Starting filter_and_get_orders_workflow")
    ...
    # Step 1: Set 'delivered'
    filter_res = set_order_status_filters(driver, timeouts)
    if not filter_res["success"]:
        result["error"] = f"set_order_status_filters failed: {filter_res['error']}"
        return result
