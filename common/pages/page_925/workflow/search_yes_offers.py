# /common/pages/page_925/workflow/search_yes_offers.py
import logging

# Import individual actions
from common.pages.page_925.actions.select_yes_offers import select_yes_offers
...
@jenkins_aware()
def search_yes_offers_workflow(driver, timeouts=None):
    """
    Complete workflow to search for offers with 'Yes' condition:
    ...
    logger.info("Starting workflow: search for offers with 'Yes' condition")
    ...
    # Step 1: Select 'Yes' radio button
    logger.info("Step 1: Select 'Yes' radio button")
    select_result = select_yes_offers(driver, timeouts)
    if not select_result["success"]:
        result["failed_step"] = "select_yes_offers"
        result["error"] = f"Failed to select 'Yes' radio button: {select_result['error']}"
        return result
    
    result["steps_completed"].append("select_yes_offers")
