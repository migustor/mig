...
# Workflows
# These workflows for SO and PO only differ by the filter file used:
...
from projects.gr_eu.pages.page_907.workflow.order_filter_workflow_po import filter_and_get_orders_workflow_po as po_filter_workflow
...
logger = logging.getLogger(" - COMBINED TEST - ")
...
def run_so_test(driver):
    """
    This function performs the SO part of the test:
...
def run_po_test(driver):
    """
    This function performs the PO part of the test:
      1) Go to page_907 with ?order_type=po
      2) Apply filter (Waiting for Pick-Up / Arrival), build report, get first 3 orders
      3) Try each order ID with email check and validation until one succeeds
    """
    logger.info("[PO TEST] Navigating to 907 (PO)")
    result = {
        "success": False,
        "steps": {}
    }

    # Step 1: Navigate to 907 with order_type=po
    page_907_url = get_page_907_url("gr_eu", "po")
    driver.get(page_907_url)
    time.sleep(3)
    logger.info(f"[PO TEST] Current URL: {driver.current_url}")
    result["steps"]["navigate_907_po"] = "SUCCESS"

    # Step 2: Filter (Waiting for Pick-Up / Arrival), build report, get orders
    filter_res = po_filter_workflow(driver, timeouts=TIMEOUTS)
    if not filter_res["success"]:
        logger.error(f"[PO TEST] Filter workflow failed: {filter_res['error']}")
        result["steps"]["po_filter"] = "FAILED"
        return result
    result["steps"]["po_filter"] = "SUCCESS"

    order_ids = filter_res.get("order_ids", [])
    if not order_ids:
        logger.error("[PO TEST] No orders found.")
        result["steps"]["po_orders"] = "FAILED"
        return result
    
    logger.info(f"[PO TEST] Found {len(order_ids)} order IDs: {order_ids}")
    result["steps"]["po_orders"] = "SUCCESS"
    ...
    for order_id in order_ids:
        logger.info(f"[PO TEST] Trying complete workflow for order ID: {order_id}")
        order_result = try_order_workflow(driver, order_id, TIMEOUTS)
        all_order_results.append(order_result)
        
        if order_result["success"]:
            logger.info(f"[PO TEST] Order ID {order_id} completed successfully")
            result["success"] = True
            result["order_id"] = order_id
            return result
    ...
    # If we're here, all orders failed or were skipped
    if all_skipped:
        logger.warning("[PO TEST] All orders were skipped (Generate Email button not available)")
        result["steps"]["po_validation"] = "SKIPPED"
    else:
        logger.error("[PO TEST] All orders failed validation or email check")
        result["steps"]["po_validation"] = "FAILED"
...
def main():
    ...
    Main entry point that:
     ...
     6) Logs out and quits
    """
    headless = False  # Adjust if needed
    driver = setup_chrome_driver(headless=headless, test_id="combined_orders_test")
    ...
    try:
        ...
        # PO part
        po_result = run_po_test(driver)
        final_result["details"].append(("PO", po_result))
        ...
        if final_result["success"]:
            print("=== TEST PASSED ===")
            print("Both SO and PO checks passed successfully.")
        else:
            print("=== TEST FAILED ===")
